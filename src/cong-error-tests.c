			      ...
			      GnomeVFSResult vfs_result)
{
	...
	dialog = cong_error_dialog_new_from_file_open_failure_with_vfs_result(parent_window, vfs_uri, vfs_result);
	cong_error_dialog_run(GTK_DIALOG(dialog));		
	gtk_widget_destroy(GTK_WIDGET(dialog));    

	g_message(_("Testing File->Save failure of \"%s\" with result %s"), filename, gnome_vfs_result_to_string(vfs_result));

	dialog = cong_error_dialog_new_from_file_save_failure(parent_window, vfs_uri, vfs_result, &file_size);
	cong_error_dialog_run(GTK_DIALOG(dialog));		
	gtk_widget_destroy(GTK_WIDGET(dialog));    

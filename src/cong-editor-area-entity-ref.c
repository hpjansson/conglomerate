				       ...
				       const gchar *text)
{
	...
	cong_editor_area_bin_construct (CONG_EDITOR_AREA_BIN(area_entity_ref),
					editor_widget);

	PRIVATE(area_entity_ref)->outer_hcompose = cong_editor_area_composer_new (editor_widget,
	...
	cong_editor_area_composer_pack_end (CONG_EDITOR_AREA_COMPOSER(PRIVATE(area_entity_ref)->outer_hcompose),
					    PRIVATE(area_entity_ref)->title_text,
					    FALSE,
					    FALSE,
					    0);


	PRIVATE(area_entity_ref)->inner_bin = cong_editor_area_bin_new (editor_widget);

	cong_editor_area_composer_pack_end (CONG_EDITOR_AREA_COMPOSER(PRIVATE(area_entity_ref)->outer_hcompose),
					    PRIVATE(area_entity_ref)->inner_bin,
					    TRUE,
					    TRUE,
					    0);


	cong_editor_area_protected_postprocess_add_internal_child (CONG_EDITOR_AREA (area_entity_ref),
								   PRIVATE(area_entity_ref)->outer_hcompose);

	cong_editor_area_protected_set_parent (PRIVATE(area_entity_ref)->outer_hcompose,
					       CONG_EDITOR_AREA (area_entity_ref));
...

{
	...
	g_message("cong_editor_area_entity_ref_new(%s)", text);
#endif

	g_return_val_if_fail (editor_widget, NULL);
	g_return_val_if_fail (text, NULL);

	return cong_editor_area_entity_ref_construct
		(g_object_new (CONG_EDITOR_AREA_ENTITY_REF_TYPE, NULL),
		 editor_widget,
		 text);
}
...
#if 0
	GdkGC *gc;
	CongEditorAreaEntityRef *area_entity_ref = CONG_EDITOR_AREA_ENTITY_REF(area);
	CongDispspecElement *ds_element = PRIVATE(area_entity_ref)->ds_element;
...
#if 1
	title_bar_height = cong_editor_area_get_cached_requisition (PRIVATE(area_entity_ref)->title_vcompose,
								    GTK_ORIENTATION_VERTICAL);
#else
	title_req = cong_editor_area_get_cached_requisition (PRIVATE(area_entity_ref)->title_vcompose);
	g_assert(title_req);
...
calc_requisition (CongEditorArea *area, 
		  ...
		  int width_hint)
{
	CongEditorAreaEntityRef *entity_ref = CONG_EDITOR_AREA_ENTITY_REF(area);

	return cong_editor_area_get_requisition (PRIVATE(entity_ref)->outer_hcompose,
						 orientation,
						 width_hint);
}
...
allocate_child_space (CongEditorArea *area)
{
	...
	cong_editor_area_set_allocation (PRIVATE(entity_ref)->outer_hcompose,
					 rect->x,
					 rect->y,
					 rect->width,
					 rect->height);
...
for_all (CongEditorArea *editor_area, 
	 ...
	 gpointer user_data)
{
	CongEditorAreaEntityRef *entity_ref = CONG_EDITOR_AREA_ENTITY_REF(editor_area);

	if ((*func)(PRIVATE(entity_ref)->outer_hcompose, user_data)) {
		return PRIVATE(entity_ref)->outer_hcompose;
	}
